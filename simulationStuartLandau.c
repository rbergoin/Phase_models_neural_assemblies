// gcc -W -Wall -o simulationStuartLandau simulationStuartLandau.c
// ./simulationStuartLandau

#include "stuartLandau.h"
...
{	
	...
	double g; 		 				//the coupling strength
	float ratio;					//ratio of excitatory oscillators
	char alphaPolicy;				//type of policy for the bifurcation parameter
	...
	char statePolicy;				//Type of policy for the phases array creation
	...
	else
	{
		...
		dt = atof(argv[5]);
		g = atof(argv[6]);
		alphaPolicy = argv[7][0];
		adjacencyPolicy = argv[8];
		weightPolicy = argv[9][0];
		frequencyPolicy = argv[10][0];
		statePolicy = argv[11][0];
		ratio = atof(argv[12]);
		save = atoi(argv[13]);
	}
	...
	{
		oscillators = initOscillators(n, epsilon1, epsilon2, dt, g, alphaPolicy, adjacencyPolicy, weightPolicy, frequencyPolicy, statePolicy, ratio); 		//Create a network of n oscillators
	}
    ...
    {
		fprintf(fptr,"%3.5Lf ", cargl(oscillators.z[i]));
	}
	fprintf(fptr, "\n");
